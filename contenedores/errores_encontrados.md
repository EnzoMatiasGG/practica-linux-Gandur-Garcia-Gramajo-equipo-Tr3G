...
**Problema:**
Al revisar la interfaz de Prometheus en `http://localhost:9090/targets`, el target de `nginx` aparece en estado **DOWN (Rojo)** con un error de conexion.
* **Causa:** En el archivo `prometheus.yml` se configuro a Prometheus para buscar métricas en `nginx:9113`. Sin embargo, la imagen oficial `nginx:alpine` que estamos usando no expone métricas en ese puerto de forma nativa (se requeriría un *exporter* adicional).
